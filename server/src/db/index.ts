...
const deleteTurtleStatement = db.prepare('DELETE FROM `turtles` WHERE `server_id` = ? AND `id` = ?');
...
const setTurtleFuel = db.prepare('UPDATE `turtles` SET `fuel_level` = ? WHERE `server_id` = ? AND `id` = ?');
