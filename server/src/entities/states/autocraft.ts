import {Inventory} from '../../db/turtle.type';
...
export class TurtleAutocraftState extends TurtleBaseState<AutocraftStateData> {
    ...
    public async *act() {
        ...
        const [didTestCraft, testCraftMessage] = await this.turtle.usePeripheralWithName<[boolean, string]>(
            'workbench',
            'craft',
            '0'
        );
        if (!didTestCraft) {
            throw new Error(testCraftMessage);
        }
        ...
        while (true) {
            ...
            for await (const _ of this.pullItemsFromNearbyInventories()) {
                yield;
            }
        }
    }

    private async *pullItemsFromNearbyInventories(): AsyncGenerator<void> {
        if (
