...
export const useBlockScanner = async (turtle: Turtle): Promise<(Block & Location)[]> => {
    const [scannedBlocks, scanMessage] = await turtle.usePeripheralWithName<[(Block & Location)[], string]>(
        'plethora:scanner',
        'scan'
    );
...
export const useUniversalScanner = async (turtle: Turtle): Promise<(Block & Location)[]> => {
    const [scannedBlocks, scanMessage] = await turtle.usePeripheralWithName<[(Block & Location)[], string]>(
        'universal_scanner',
        'scan',
        '"block"',
...
export async function* geoScannerCooldown(turtle: Turtle) {
    let [cooldown] = await turtle.usePeripheralWithName<[number]>('geoScanner', 'getOperationCooldown', '"scanBlocks"');
    yield;
...
export const useGeoScanner = async (turtle: Turtle): Promise<(Block & Location)[]> => {
