      ...
      fetch(
        ...
        .then((body) => {
          ...
          if (mainPictures != null) {
            const pictureChild =
              mainPictures.firstElementChild as HTMLLinkElement;
        ...
        .then(({ image, renderObject }) => {
          ...
          const grabcraftNameRegexp = new RegExp(
            "([\\d\\w-_ ]+) ?(\\([^)]+\\))?",
          );
          for (const y in renderObject) {
            for (const x in renderObject[y]) {
              for (const z in renderObject[y][x]) {
                const capturedGrabcraftName = grabcraftNameRegexp.exec(
                  renderObject[y][x][z].name,
                );
                ...
                const name = (() => {
                  const blockName =
                    grabcraftNameToBlockNameMap.get(grabcraftName);
                  if (blockName != null) {
                    return blockName;
                  }
                  ...
                  grabcraftNameToBlockNameMap.set(
                    grabcraftName,
                    bestStringMatch,
                  );
        ...
        <Form validated={isFormValidated} onSubmit={handleFormSubmit}>
          ...
          <Row className="mb-3">
            <Btn
              ...
              type="submit"
              disabled={isLoading || buildData != null}
            >
              Load
            </Btn>
          </Row>
          {buildData != null ? (
            <Row>
              {buildData.image != null ? <img src={buildData.image} /> : null}
            </Row>
          ) : null}
        </Form>
      </Modal.Body>
