...
import { AtlasMap, AtlasMapBlockState } from "../../../../hooks/useAtlasMap";
...
const BuildMeshDataFromVoxels = (
  ...
  for (const [key, cell] of cells) {
    ...
    const matchingBlock = atlasMapBlock?.reduce(
      (bestMatch, block, index) => {
        ...
        for (const stateKey of stateKeys) {
          if (cellState?.[stateKey]?.toString() ?? "false" !== state[stateKey])
            return bestMatch;
        }
    ...
    const blockTextures =
      (matchingBlock != null ? atlasMap.textures[matchingBlock.model] : null) ??
      unknownTexture;
    ...
    for (let i = 0; i < blockFaces.length; i++) {
      ...
      const localPositions = (() => {
        ...
        if (matchingBlock.x == null && matchingBlock.y == null)
...
const SparseBlock = forwardRef<SparseBlockHandle, Props>(
  function SparseBlock(props, ref) {
