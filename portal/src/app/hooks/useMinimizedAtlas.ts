...
export const useMinimizedAtlas = (
  ...
  useMemo(() => {
    ...
    const collectTextures = (model: Model, noNamespace?: boolean) => {
      ...
      const parentModel =
        models[
          parentNamespaceSplit.length > 1 ? parent : `minecraft:${parent}`
        ];
    ...
    for (const block of uniqueBlocks.values()) {
      const blockstate = blockstates[block.name];
      ...
      const modelNames = (() => {
        if (variants != null) {
          ...
          if (keys.length < 1) {
            console.log(`${block.name} is missing variant keys!`);
            return;
          }
      ...
      if (modelNames == null) {
        console.warn(`Cannot find model name for ${block.name}`);
        continue;
      }
      ...
      for (const modelName of modelNames) {
        ...
        if (model == null) {
          console.log(
            `No model with name ${modelName} exists for ${block.name}`,
          );
          continue;
        }
