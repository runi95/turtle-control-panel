...
export type IsBlockMineableFunc = (
  x: number,
  y: number,
  z: number,
  block: Block | null,
...
export default class DStarLite {
  ...
  public async search(
    ...
    if (
      destinations.some(
        (d) => d.x === source.x && d.y === source.y && d.z === source.z,
      )
    )
      return null;
    ...
    for (const destination of destinations) {
      const destinationNode = this.getCachedNode(
        cachedNodes,
        destination.x,
        destination.y,
        destination.z,
      ...
      destinationNode.key = [
        heuristic.calculate(startNode.point, destinationNode.point),
        0,
      ];
    ...
    logger.debug(
      `No valid path to: ${destinations.map(({ x, y, z }) => `(${x},${y},${z})`).join(", ")}`,
    );
  ...
  private c(u: Node, v: Node): number {
    if ((u.isWall && !u.isMineable) || (v.isWall && !v.isMineable))
  ...
  private getCachedNode(
    ...
    if (
      ...
      const node = new Node(
        new Point(x, y, z),
        false,
        this.isBlockMineableFunc !== null
          ? this.isBlockMineableFunc(x, y, z, block)
          : false,
      );
  ...
  private succ(u: Node, cachedNodes: Map<string, Node>): Node[] {
    ...
    if (
      this.boundaries?.maxX == null ||
      u.point.x + 1 <= this.boundaries.maxX
    ) {
      const east = this.getCachedNode(
        cachedNodes,
        u.point.x + 1,
        u.point.y,
        u.point.z,
    ...
    if (
      u.point.y < 255 &&
      (this.boundaries?.maxY == null || u.point.y + 1 <= this.boundaries.maxY)
    ) {
      const up = this.getCachedNode(
        cachedNodes,
        u.point.x,
        u.point.y + 1,
        u.point.z,
    ...
    if (
      this.boundaries?.maxZ == null ||
      u.point.z + 1 <= this.boundaries.maxZ
    ) {
      const south = this.getCachedNode(
        cachedNodes,
        u.point.x,
        u.point.y,
        u.point.z + 1,
    ...
    if (
      this.boundaries?.minX == null ||
      u.point.x - 1 >= this.boundaries.minX
    ) {
      const west = this.getCachedNode(
        cachedNodes,
        u.point.x - 1,
        u.point.y,
        u.point.z,
    ...
    if (
      u.point.y > -59 &&
      (this.boundaries?.minY == null || u.point.y - 1 >= this.boundaries.minY)
    ) {
      const down = this.getCachedNode(
        cachedNodes,
        u.point.x,
        u.point.y - 1,
        u.point.z,
    ...
    if (
      this.boundaries?.minZ == null ||
      u.point.z - 1 >= this.boundaries.minZ
    ) {
      const north = this.getCachedNode(
        cachedNodes,
        u.point.x,
        u.point.y,
        u.point.z - 1,
