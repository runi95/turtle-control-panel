...
export const prepareSelectBlocksWithNameLike = (db: Database) =>
    db
        .prepare(
            `SELECT json_object(
'serverId', \`b\`.\`server_id\`,
...
ON \`lb\`.\`server_id\` = \`b\`.\`server_id\` AND \`lb\`.\`x\` = \`b\`.\`x\` AND \`lb\`.\`max\` = \`b\`.\`y\` AND \`lb\`.\`z\` = \`b\`.\`z\`
WHERE \`b\`.\`server_id\` = :server_id AND \`b\`.\`x\` >= :from_x AND \`b\`.\`x\` <= :to_x AND \`b\`.\`y\` >= :from_y AND \`b\`.\`y\` <= :to_y AND \`b\`.\`z\` >= :from_z AND \`b\`.\`z\` <= :to_z AND \`b\`.\`name\` LIKE :name`
        )
        .pluck();
