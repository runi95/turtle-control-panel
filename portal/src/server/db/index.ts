...
import {
  ...
  prepareInsertServer,
  prepareUpdateServer,
} from "./queries/queries/servers";
import {
  prepareInsertArea,
  prepareSelectArea,
  prepareSelectAreas,
} from "./queries/queries/areas";
import {
  prepareDeleteBlock,
  prepareInsertBlock,
  prepareSelectBlock,
...
const setTurtleStepsSinceLastRefuel =
  prepareUpdateTurtleStepsSinceLastRefuel(db);
...
export const getDashboard = () =>
  preparedDashboard
    .all()
    .map((server: unknown) => JSON.parse(server as string));
export const upsertServer = (remoteAddress: string, name: string | null) =>
  insertServer.run({
    remote_address: remoteAddress,
    name,
  });
export const renameServer = (id: number, name: string) =>
  setServerName.run(name, id);
export const getServerByRemoteAddress = (remoteAddress: string) =>
  selectServerByRemoteAddress.get(remoteAddress) as Server;
export const getArea = (serverId: number, id: number) =>
  JSON.parse(selectArea.get(serverId, id) as string) as Area;
export const getAreas = (serverId: number) =>
  JSON.parse(selectAreas.get(serverId) as string) as Area[];
...
export const getTurtle = (serverId: number, id: number) => {
  ...
  return turtleString === undefined
    ? null
    : (JSON.parse(turtleString) as Turtle);
};
...
export const getBlocksWithNameLike = (
  serverId: number,
  options: GetBlocksOptions & { name: string },
) =>
...
export const deleteBlocks = (
  ...
  db.transaction(() => {
    for (const block of blocks)
      deleteBlock(serverId, block.x, block.y, block.z);
  })();
export const deleteTurtle = (serverId: number, id: number) =>
  deleteTurtleStatement.run(serverId, id);
export const updateTurtleName = (serverId: number, id: number, name: string) =>
  setTurtleName.run(name, serverId, id);
export const updateTurtleFuelLevel = (
  serverId: number,
  id: number,
  fuelLevel: number,
) => setTurtleFuelLevel.run(fuelLevel, serverId, id);
export const updateTurtleSelectedSlot = (
  serverId: number,
  id: number,
  selectedSlot: number,
) => setTurtleSelectedSlot.run(selectedSlot, serverId, id);
export const updateTurtleInventory = (
  serverId: number,
  id: number,
  inventory: Inventory,
) => setTurtleInventory.run(JSON.stringify(inventory), serverId, id);
export const updateTurtleStepsSinceLastRefuel = (
  serverId: number,
  id: number,
  stepsSinceLastRefuel: number,
...
export const updateTurtleLocation = (
  serverId: number,
  id: number,
  location: Location | null,
) => setTurtleLocation.run(JSON.stringify(location), serverId, id);
export const updateTurtleDirection = (
  serverId: number,
  id: number,
  direction: Direction | null,
...
export const updateTurtleFuel = (
  serverId: number,
  id: number,
  fuelLevel: number,
) => setTurtleFuel.run(fuelLevel, serverId, id);
export const updateTurtleHome = (
  serverId: number,
  id: number,
  home: Location | null,
