...
import {
  ExtractionStateData,
  TurtleExtractionState,
} from "./states/extraction";
...
export const onConnect = (ws: WebSocket, req: IncomingMessage) => {
  ...
  const messageConstructor = async (msg: Buffer) => {
    ...
    if (msg[0] !== 0x01) {
      logger.warning(
        `Turtle WebSocket message does not start with 0x01 (start of heading)`,
      );
      return;
    }
    ...
    const str = msg.toString(
      "utf-8",
      10 + uuidLength,
      isFinalMessage ? msg.length - 1 : undefined,
    );
    if (isFinalMessage) {
      const obj = JSON.parse(
        messageIndex === 1
          ? str
          : Object.values(
              messageConstructorMap.get(
                messageUuid,
              ) as MessageConstructorObject,
            ).reduce((acc, curr) => acc + curr, "") + str,
      );
    ...
    const messageConstructorObject = messageConstructorMap.get(
      messageUuid,
    ) as MessageConstructorObject;
  ...
  (() => {
    ...
    ws.send(
      JSON.stringify({ type: "HANDSHAKE", uuid, logLevel: turtleLogLevel }),
    );
    const handshake = (obj: {
      type: "HANDSHAKE";
      message: {
      ...
      const { stepsSinceLastRefuel, state, location, direction, home } =
        getTurtle(serverId, id) ?? {
...
export class Turtle {
  ...
  private lastPromise: Promise<unknown> = new Promise<void>((resolve) =>
    resolve(),
  );
  ...
  constructor(
    ...
    this.turtleEventEmitter.on(
      "update",
      (obj: { type: string; message: unknown }) => {
        ...
        switch (type) {
          ...
          case "PERIPHERAL_DETACHED":
            this.peripherals = (({ [message as string]: _, ...peripherals }) =>
  ...
  private getRecoveredState(
    data: StateDataTypes | null,
  ): TurtleBaseState<StateDataTypes> | null {
  ...
  > {
    ...
    if (didMove) {
      this.#fuelLevel--;
      this.#stepsSinceLastRefuel++;
      const [xChange, zChange] = getLocalCoordinatesForDirection(
        this.direction,
      );
  ...
  async turnLeft(): Promise<[true, undefined] | [false, "Unknown direction"]> {
    const turnLeft = await this.#exec<
      [true, undefined] | [false, "Unknown direction"]
    >("turtle.turnLeft()");
  ...
  async turnRight(): Promise<[true, undefined] | [false, "Unknown direction"]> {
    const turnRight = await this.#exec<
      [true, undefined] | [false, "Unknown direction"]
    >("turtle.turnRight()");
  ...
  async refreshInventoryState(
    from: number = this.selectedSlot,
    stopOnUndefined: boolean = false,
  ): Promise<void> {
  ...
  async inspect(): Promise<Block | null | undefined> {
    ...
    const [didInspect, block] = await this.#exec<
      [true, Block] | [false, "No block to inspect"]
    >("turtle.inspect()");
    if (!didInspect) {
  ...
  async drop(
    ...
    [true, undefined] | [false, "No space for items" | "No items to drop"]
  > {
    const drop = await this.#exec<
      [true, undefined] | [false, "No space for items" | "No items to drop"]
  ...
  async dropDown(
    ...
    [true, undefined] | [false, "No space for items" | "No items to drop"]
  > {
    const dropDown = await this.#exec<
      [true, undefined] | [false, "No space for items" | "No items to drop"]
  ...
  async select(slot: number = 1): Promise<[boolean, string | undefined]> {
    const select = await this.#exec<[boolean, string | undefined]>(
      `turtle.select(${slot})`,
    );
  ...
  async suck(
    ...
    | [true, undefined]
    | [false, "No space for items" | "No items to drop" | "No items to take"]
  > {
  ...
  async refuel(
    ...
    [true, undefined] | [false, "No items to combust" | "Items not combustible"]
  > {
    const refuel = await this.#exec<
      | [true, undefined]
      | [false, "No items to combust" | "Items not combustible"]
    >(count ? `turtle.refuel(${count})` : "turtle.refuel()");
    const [didRefuel] = refuel;
    if (didRefuel) {
      const [updatedFuelLevel] = await this.getFuelLevel();
      this.#fuelLevel =
        typeof updatedFuelLevel === "string"
  ...
  async transferTo(
    slot: number,
    count?: number,
  ): Promise<[true, undefined] | [false, "No space for items"]> {
  ...
  async craft(): Promise<void> {
    ...
    const [didCraft, craftMessage] = await this.#exec<
      [true, undefined] | [false, "No matching recipes"]
  ...
  async hasPeripheralWithName(peripheralName: string): Promise<[boolean]> {
    return await this.#exec<[boolean]>(
      `peripheral.find("${peripheralName}") ~= nil`,
    );
  }

  async usePeripheralWithName<R>(
    peripheralName: string,
    method: string,
    ...args: string[]
  ): Promise<R> {
    return await this.#exec<R>(
      `peripheral.find("${peripheralName}").${method}(${args?.join(", ")})`,
    );
  }
   ...
   */
  async attack(): Promise<
    | [true, undefined]
    | [false, "Nothing to attack here" | "No tool to attack with"]
  > {
    return await this.#exec<
      | [true, undefined]
      | [false, "Nothing to attack here" | "No tool to attack with"]
  ...
  async getFuelLevel(): Promise<[number]> {
    const [updatedFuelLevel] = await this.#exec<[number | string]>(
      "turtle.getFuelLevel()",
    );
    this.fuelLevel =
      typeof updatedFuelLevel === "string"
