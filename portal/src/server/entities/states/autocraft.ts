...
export class TurtleAutocraftState extends TurtleBaseState<AutocraftStateData> {
  ...
  public async *act() {
    const [hasCraftingTable] =
      await this.turtle.hasPeripheralWithName("workbench");
    if (!hasCraftingTable)
      throw new Error("Unable to auto craft without a Crafting Table");

    const [didInitialCraft, initialCraftMessage] =
      await this.turtle.usePeripheralWithName<[boolean, string]>(
        "workbench",
        "craft",
        "0",
      );
    ...
    while (true) {
      ...
      for (let i = 1; i <= 16; i++) {
        ...
        try {
          ...
          await this.turtle.updateAllAttachedPeripherals(
  ...
  private async *pullItemsFromNearbyInventories(
    item: ItemDetail,
    slotNumber: number,
  ): AsyncGenerator<void> {
    if (
          ...
          case "back":
            ...
            if (j > 0) {
              ...
              const [pushedItemCount] = await this.turtle.usePeripheralWithSide<
                [number]
              >(side, "pushItems", side, j + 1, null, 1);
