  ...
  public async *act() {
    while (true) {
      ...
      for (const blockList of this.blocks.values()) {
        ...
        if (block == null) continue;
        if (this.disabledKeysMap.get(`${block.x},${block.z}`) === true)
          continue;

        hasAnyNonEmptyList = true;
        buildingMaterials.set(
          block.name,
          (buildingMaterials.get(block.name) ?? 0) + 1,
        );
      ...
      if (
        ...
        for (const [itemName, itemCount] of buildingMaterials.entries()) {
          try {
            for await (const _ of this.pullItemFromNearbyInventories(
              itemName,
              itemCount,
            )) {
              yield;
        ...
        for (const [
          itemName,
          itemCount,
        ] of optionalBuildingMaterials.entries()) {
          try {
            for await (const _ of this.pullItemFromNearbyInventories(
              itemName,
              itemCount,
      ...
      const inventoryItems = (
        Object.values(this.turtle.inventory) as ItemDetail[]
      ).reduce((acc, curr) => {
