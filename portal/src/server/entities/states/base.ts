...
export abstract class TurtleBaseState<T> {
  ...
  protected async *refuelFromNearbyInventories(): AsyncGenerator<void> {
    const { inventories, hubs } = Object.entries(
      this.turtle.peripherals,
    ).reduce(
      (acc, [side, { types, data }]) => {
    ...
    for (const [side, peripheral] of inventories) {
        ...
        switch (side) {
          ...
          default:
            return hubs.some(([_, { data }]) => {
              ...
              return (data as { remoteNames: string[] })?.remoteNames?.includes(
                side,
              );
            });
        }
      ...
      const { content } = data as {
        content: { name: string; tags?: { [key: string]: boolean } }[] | null;
      };
      ...
      switch (item.side) {
        ...
        default:
          const connectedHub = hubs.find(([_, { data }]) =>
            (data as { remoteNames: string[] })?.remoteNames?.includes(
              (item as ComparableItem).side,
            ),
          );
          if (connectedHub) {
            ...
            const [pulledItemCount] = await this.turtle.usePeripheralWithSide<
              [number]
            >(
              item.side,
              "pushItems",
  ...
  protected async *pullItemFromNearbyInventories(
    itemName: string,
    count: number,
  ): AsyncGenerator<void> {
    if (
        ...
        switch (side) {
          ...
          case "back":
            ...
            if (i > 0) {
              ...
              const [pushedItemCount] = await this.turtle.usePeripheralWithSide<
                [number]
              >(side, "pushItems", side, i + 1, null, 1);
  ...
  protected async *transferSlotIntoNearbyInventories(
    slot: number,
  ): AsyncGenerator<void> {
    ...
    switch (bestMatchingSide) {
      ...
      default:
        ...
        if (connectedHub) {
          ...
          if (pulledItemCount < 1)
            throw new Error(`Unable to transfer to '${bestMatchingSide}'`);
  ...
  protected async *goToDestinations(
    ...
    let solution = await algorithm.search(
      new Point(location.x, location.y, location.z),
      destinations,
    );
    if (solution === null) {
      yield;
    ...
    while (solution !== null) {
      const [didMoveToNode, failedMoveMessage] = await this.moveToNode(
        solution,
        options?.noInspect,
      );
      if (didMoveToNode) {
  ...
  protected async moveToNode(
    ...
    if (s.point.y - y > 0) {
      ...
      if (noInspect || upMessage !== "Movement obstructed")
        return [false, upMessage as string];
      ...
      const [didMoveUpAfterDig, moveDownAfterDigMessage] =
      ...
      // We don't know what the other potential errors are!
      if (noInspect || downMessage !== "Movement obstructed")
        return [false, downMessage as string];
      ...
      const [didMoveDownAfterDig, moveDownAfterDigMessage] =
      ...
      const direction =
        heading.x +
        Math.abs(heading.x) * 2 +
        (heading.z + Math.abs(heading.z) * 3);
      ...
      // We don't know what the other potential errors are!
      if (noInspect || forwardMessage !== "Movement obstructed")
        return [false, forwardMessage as string];
      ...
      const [didMoveForwardAfterDig, forwardMessageAfterDig] =
  ...
  protected async selectItemOfType(name: string) {
    const itemOfType = Object.entries(this.turtle.inventory).find(
      ([_, item]) => item?.name === name,
    );
