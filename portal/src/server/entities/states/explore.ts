...
export class TurtleExploringState extends TurtleBaseState<ExploreStateData> {
  ...
  private async exploreSurroundings(
    x: number,
    y: number,
    z: number,
    locationsToExplore: Location[],
  ) {
    const locationIndex = locationsToExplore.findIndex(
      (loc) => loc.x === x && loc.y === y && loc.z === z,
    );
    if (locationIndex > 0) {
    ...
    const upIndex = locationsToExplore.findIndex(
      (loc) => loc.x === x && loc.y === y + 1 && loc.z === z,
    );
    if (upIndex > -1) {
    ...
    const downIndex = locationsToExplore.findIndex(
      (loc) => loc.x === x && loc.y === y - 1 && loc.z === z,
    );
    if (downIndex > -1) {
    ...
    const frontIndex = locationsToExplore.findIndex(
      (loc) => loc.x === x + dx && loc.y === y && loc.z === z + dz,
    );
    if (frontIndex > -1) {
      ...
      while (locationsToExplore.length > 0) {
        try {
          ...
          if (
            ...
            err instanceof DestinationError &&
            (err.message === "No valid path found" ||
              err.message === "Max steps reached")
          ) {
            const { x, y, z } = this.turtle.location;
            await this.exploreSurroundings(x, y, z, locationsToExplore);
