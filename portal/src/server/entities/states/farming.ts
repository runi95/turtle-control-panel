...
export class TurtleFarmingState extends TurtleBaseState<FarmingStateData> {
  ...
  public async *act() {
    while (true) {
      ...
      if (!this.isInFarmingArea) {
        ...
        if (
          this.area.some(
            ({ x: areaX, y: areaY, z: areaZ }) =>
              areaX === x && areaY === y && areaZ === z,
          )
        ) {
      ...
      if (farmlandIndexOfBlock > -1) {
        ...
        if (block === null) {
          ...
          const farmingBlockToFarmingDetails =
            blockToFarmingDetailsMapObject[block.name];
          if (farmingBlockToFarmingDetails) {
            ...
            if (
              fuelPercentage < 10 ||
              !this.hasSpaceForItem(farmingBlockToFarmingDetails.harvest)
            ) {
  ...
  private async selectAnySeedInInventory() {
    const inventoryEntry = Object.entries(this.turtle.inventory).find(
      ([_, item]) => farmingSeedNames.includes(item?.name),
    );
