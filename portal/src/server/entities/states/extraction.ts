...
export class TurtleExtractionState extends TurtleBaseState<ExtractionStateData> {
  ...
  private readonly mineableBlockIncludeOrExcludeMap = new Map<
    string,
    boolean
  >();
  ...
  constructor(turtle: Turtle, data: Omit<ExtractionStateData, "id">) {
    ...
    this.hasExclusions =
      this.isInExcludeMode && includeOrExcludeList.length > 0;
  ...
  private findBestScanIndex(
    remainingScanAreaIndexes: number[],
    scanRadius: number = 16,
  ) {
    const { minX, minY, minZ, maxX, maxY, maxZ } = this.getBoundingBox(
      remainingScanAreaIndexes,
    );
  ...
  private async initializeScanIndexes() {
    ...
    while (remainingScanAreaIndexes.length > 0) {
      const scanIndex = this.findBestScanIndex(
        remainingScanAreaIndexes,
        scanRadius,
      );
  ...
  public async *act() {
    while (true) {
      ...
      if (
        this.scanIndexes.length === 0 &&
        this.remainingAreaIndexes.length === 0
      )
        return; // Done!
      ...
      if (matchingScanIndex > -1) {
        ...
        for (const block of blocks) {
          if (this.isInExcludeMode) {
            ...
            if (
              this.hasExclusions &&
              !!this.mineableBlockIncludeOrExcludeMap.get(block.name)
            )
              continue;
          } else if (!this.mineableBlockIncludeOrExcludeMap.get(block.name))
            continue;
          ...
          if (
            areaIndex > -1 &&
            !this.remainingAreaIndexes.includes(areaIndex)
          ) {
            this.remainingAreaIndexes.push(areaIndex);
          }
        }
      ...
      const areaIndexOfTurtle = this.remainingAreaIndexes.findIndex(
        (i) =>
          this.area[i].x === x && this.area[i].y === y && this.area[i].z === z,
      );
      ...
      try {
        for await (const _ of this.goToDestinations(
          ...
          {
            isBlockMineableFunc: (x, y, z, _block) =>
              !!this.mineableBlockMap.get(`${x},${y},${z}`),
          },
        )) {
          yield;

          // Go home?
          const hasAvailableSpaceInInventory = Object.values(
            this.turtle.inventory,
          ).some((value) => value == null);
          const fuelPercentage =
            (100 * this.turtle.fuelLevel) / this.turtle.fuelLimit;
          if (fuelPercentage < 10 || !hasAvailableSpaceInInventory) {
            ...
            try {
              for await (const _ of this.goToDestinations([
                new Point(home.x, home.y, home.z),
              ])) {
                yield;
              }
            } catch (err) {
              if (
                ...
                err.message === "Movement obstructed"
              ) {
                yield;
                continue;
        ...
        if (
          err instanceof DestinationError &&
          (err.message === "Movement obstructed" ||
            err.message === "Cannot break unbreakable block")
        ) {
