...
const turtleWssPort = process.env.TURTLE_WSS_PORT
  ? Number(process.env.TURTLE_WSS_PORT)
  : 5757;
...
const startServer = async () => {
  ...
  logger.info(
    `Turtle WebSocket listening on port \x1b[36m${turtleWssPort}\x1b[0m`,
  );

  turtleWebsocketServer.on("connection", (ws, req) => {
    onConnect(ws, req);
  });
      ...
      try {
        switch (msg.type) {
          case "HANDSHAKE":
            ...
            break;
          case "ACTION":
            const turtle = getOnlineTurtleById(
              Number(msg.data.serverId),
              Number(msg.data.id),
            );
            if (turtle === undefined) {
              logger.error(
            ...
            switch (msg.action) {
              ...
              case "explore":
                if (turtle.location == null) {
                  throw new Error(
                    "Unable to explore without knowing turtle location",
                  );
                } else {
              ...
              case "analyze":
                const [hasGeoScanner] =
                  await turtle.hasPeripheralWithName("geoScanner");
                if (hasGeoScanner) {
                  const [analysis, analysisFailMessage] =
                    await turtle.usePeripheralWithName<
                  ...
                  const [hasUltimateSensor] =
                    await turtle.hasPeripheralWithName("ultimate_sensor");
                  if (hasUltimateSensor) {
                    const [analysis, analysisFailMessage] =
                      await turtle.usePeripheralWithName<
                        | [
                            {
                              oresDistribution: { [key: string]: number };
                              isSlime: boolean;
                            },
                            undefined,
                          ]
              ...
              case "inventory-transfer":
                await turtle.inventoryTransfer(
                  msg.data.fromSlot,
                  msg.data.toSlot,
                  msg.data.count,
                );
              ...
              case "inventory-push-items":
                ...
                if (fromSide === "") {
                  fromTurtle = true;
                  const hub = Object.entries(turtle.peripherals).find(
                    ([_side, { types, data }]) => {
                      if ((data as any)?.remoteNames === undefined)
                        return false;
                      if ((data as any)?.localName === undefined) return false;
                      if (types.includes("peripheral_hub")) {
                        return (
                          data as { remoteNames: string[] }
                        ).remoteNames.includes(toSide);
                      }
                    },
                  );
                  if (hub === undefined) {
                    throw new Error(
                      `Cannot inventory-push-items from <turtle> to [${toSide}]`,
                    );
                  }
                  ...
                  const hub = Object.entries(turtle.peripherals).find(
                    ([_side, { types, data }]) => {
                      ...
                      if (types.includes("peripheral_hub")) {
                        return (
                          data as { remoteNames: string[] }
                        ).remoteNames.includes(fromSide);
                      }
                    },
                  );
                  if (hub === undefined) {
                    throw new Error(
                      `Cannot inventory-push-items from [${fromSide}] to <turtle>`,
                    );
                  }
                ...
                if (transferredItems > 0) {
                  ...
                  if (
                    msg.data.fromSide !== msg.data.toSide &&
                    msg.data.toSide !== ""
                  ) {
              ...
              case "sort-inventory":
                ...
                for (let i = 0; i < content.length; i++) {
                  ...
                  if (item.count < Number(item.maxCount)) {
                    ...
                    if (existingStack === undefined) {
                      ...
                      const [movedItems] = await turtle.usePeripheralWithSide<
                        [number]
                      >(side, "pushItems", side, i, null, existingStack);
                ...
                const sortingQueue = new PriorityQueue<ItemDetail>(
                  compareItems,
                );
                ...
                while ((item = sortingQueue.poll()) !== null) {
                  ...
                  if (itemInSlot != null) {
                    ...
                    if (!(availableSlot > -1)) {
                      ...
                      updatedContent[availableSlot - 1] =
                        updatedContent[currentSortIndex - 1];
                ...
                (
                  turtle.peripherals[side].data as {
                    content: (ItemDetail | null)[];
                  }
              ...
              case "use-peripheral":
                if (msg.data.side == null) {
                  throw new Error(
                    "Cannot call use-peripheral without a <side>",
                  );
                }

                if (msg.data.method == null) {
                  throw new Error(
                    "Cannot call use-peripheral without a <method>",
                  );
                }
              ...
              case "chatter-set-message":
                if (msg.data.side == null) {
                  throw new Error(
                    "Cannot call chatter-set-message without a <side>",
                  );
                }

                if (msg.data.message == null) {
                  throw new Error(
                    "Cannot call chatter-set-message without a <message>",
                  );
                }
              ...
              case "chatter-clear-message":
                if (msg.data.side == null) {
                  throw new Error(
                    "Cannot call chatter-clear-message without a <side>",
                  );
                }
              ...
              case "end-automata-warp":
                if (msg.data.side == null) {
                  throw new Error(
                    "Cannot call end-automata-warp without a <side>",
                  );
                }

                if (msg.data.point == null) {
                  throw new Error(
                    "Cannot call end-automata-warp without a <point>",
                  );
                }

                const regexResults = endAutomataLocationRegex.exec(
                  msg.data.point,
                );
                if (regexResults == null) {
                  throw new Error(
                    "Invalid warp <point>, location data is missing",
                  );
                }
                ...
                if (
                  ...
                  throw new Error(
                    "Invalid warp <point>, location data is corrupt",
                  );
                }
              ...
              case "end-automata-delete-point":
                if (msg.data.side == null) {
                  throw new Error(
                    "Cannot call end-automata-delete-point without a <side>",
                  );
                }

                if (msg.data.point == null) {
                  throw new Error(
                    "Cannot call end-automata-delete-point without a <point>",
                  );
                }

                const [didDelete] = await turtle.usePeripheralWithSide<
                  [boolean]
                >(msg.data.side, "deletePoint", msg.data.point);
                if (didDelete) {
              ...
              case "end-automata-save-point":
                if (msg.data.side == null) {
                  throw new Error(
                    "Cannot call end-automata-save-point without a <side>",
                  );
                }

                if (msg.data.point == null) {
                  throw new Error(
                    "Cannot call end-automata-save-point without a <point>",
                  );
                }
              ...
              default:
                logger.error(
                  `Invalid action [${msg.action}] attempted on turtle [${msg.data.id}]`,
                );
                break;
            }
            break;
          ...
          case "SERVER":
            switch (msg.action) {
              case "rename":
                ...
                globalEventEmitter.emit("supdate", {
                  id: msg.data.id,
                  name: msg.data.newName,
                });
                break;
              default:
                logger.warning(
                  `Received invalid SERVER action [${msg.action}]`,
                );
                break;
            }
            break;
          case "TURTLE":
            switch (msg.action) {
              ...
              default:
                logger.warning(
                  `Received invalid TURTLE action [${msg.action}]`,
                );
            }
            break;
          default:
  ...
  httpServer
    ...
    .listen(port, () => {
      logger.info(
        `Turtle Control Panel is ready on \x1b[36mhttp://${hostname}:${port}\x1b[0m`,
      );
    });
};
startServer();
